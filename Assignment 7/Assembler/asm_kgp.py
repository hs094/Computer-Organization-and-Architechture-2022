    ...
    with open(source_file, 'r') as f:
        for lno, line in enumerate(f.readlines()):
            ...
            if line == '' or line[0] == '#':
                ...
                op = args[0]
                if op in REFORMATS:  # r-format instructions
                    opcode = OPCODE[op]
                    funct = FUNCODE[op]
                    rs, rt = args[1], args[2]
                    shamt = 0
...
if __name__ == '__main__':
    arg = argparse.ArgumentParser(description='KGP-RISC assembler')
    arg.add_argument('-s', '--source', help='source file', required=True, type=str)
    arg.add_argument('-o', '--output', help='output file', default='a.out', type=str)
    _args = arg.parse_args()
    # Check if source file exists
    if not os.path.isfile(_args.source):
        print('Error: Source File DOES NOT EXIST in the Directory')
        sys.exit(1)
    parse_labels(_args.source)
    main(_args.source, _args.output)
